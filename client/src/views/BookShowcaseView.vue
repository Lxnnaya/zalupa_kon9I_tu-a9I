...
export default {
  ...
  async created() {
    try {
      const response = await axios.post("http://192.168.0.25:8080/book/getBook", { // Запрос к серверу на express для получения информации об учебнике
        book_id: this.$route.params.book_id,
      });
      this.book = response.data; // Объект с информацией об учебнике
      const filePath = `http://192.168.0.25:8080/static/pdf/${response.data.pdf}`; // Путь к папке с учебниками на сервере
